...
class Smart_Upsell_Admin {
    ...
    public function enqueue_scripts( $hook ) {
        ...
        if ( $screen && 'smart_upsell_rule' === $screen->post_type ) {
            ...
            wp_enqueue_script( $this->plugin_name . '-admin-rules', plugin_dir_url( __FILE__ ) . 'js/smart-upsell-admin-rules.js', [ 'jquery', 'wc-enhanced-select' ], $this->version, false );
        }
    ...
    public function display_analytics_page() {
        ...
        $conversions = $this->get_stats_by_event_type( 'conversion' );
        $conversion_rate = $this->get_conversion_rate( $impressions, $conversions );
    ...
    private function get_stats_by_event_type( $event_type ) {
        global $wpdb;
        return (int) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$wpdb->prefix}smart_upsell_stats WHERE event_type = %s", $event_type ) );
    }

    private function get_conversion_rate( $impressions, $conversions ) {
        return $impressions > 0 ? round( ( $conversions / $impressions ) * 100, 2 ) : 0;
    }

    private function get_total_revenue() {
        global $wpdb;
        return (float) $wpdb->get_var( "SELECT SUM(meta_value) FROM $wpdb->postmeta WHERE meta_key = '_revenue'" );
    }
    ...
    public function save_rules_meta_box_data( $post_id ) {
        ...
        $fields_to_sanitize = [
            ...
            '_trigger_type' => 'text',
            '_discount_type' => 'text',
            '_discount_amount' => 'float',
            '_trigger_product' => 'int',
            '_trigger_category' => 'int',
            '_upsell_product' => 'int',
        ];

        foreach ( $fields_to_sanitize as $key => $type ) {
            $post_key = substr($key, 1);
