...
class Smart_Upsell_Admin {
    ...
    public function enqueue_scripts( $hook ) {
        ...
        if ( ( 'post.php' == $hook || 'post-new.php' == $hook ) && isset($post->post_type) && 'smart_upsell_rule' == $post->post_type ) {
    ...
    public function add_admin_menu() {
        ...
        add_submenu_page(
            $this->plugin_name,
            __( 'Add New Rule', 'smart-upsell-for-woocommerce' ),
            __( 'Add New Rule', 'smart-upsell-for-woocommerce' ),
            'manage_options',
            'post-new.php?post_type=smart_upsell_rule'
        );
    ...
    public function register_settings() {
        ...
        add_settings_field( 'popup_title', __( 'Popup Title', 'smart-upsell-for-woocommerce' ), array( $this, 'render_settings_field' ), 'smart_upsell_settings', 'smart_upsell_popup_settings', [ 'type' => 'text', 'id' => 'popup_title', 'name' => 'popup_title', 'label_for' => 'popup_title', 'default' => __( 'Don\'t miss this exclusive offer!', 'smart-upsell-for-woocommerce' ) ] );
        add_settings_field( 'popup_bg_color', __( 'Popup Background Color', 'smart-upsell-for-woocommerce' ), array( $this, 'render_settings_field' ), 'smart_upsell_settings', 'smart_upsell_popup_settings', [ 'type' => 'color', 'id' => 'popup_bg_color', 'name' => 'popup_bg_color', 'label_for' => 'popup_bg_color', 'default' => '#ffffff' ] );
        add_settings_field( 'popup_button_color', __( 'Popup Button Color', 'smart-upsell-for-woocommerce' ), array( $this, 'render_settings_field' ), 'smart_upsell_settings', 'smart_upsell_popup_settings', [ 'type' => 'color', 'id' => 'popup_button_color', 'name' => 'popup_button_color', 'label_for' => 'popup_button_color', 'default' => '#0073aa' ] );
    }
    ...
    private function get_stats_by_event_type( $event_type ) {
        ...
        return $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM $table_name WHERE event_type = %s", $event_type ) );
    }
    ...
    public function register_rules_cpt() {
        ...
        $args = [ 'label' => __( 'Upsell Rule', 'smart-upsell-for-woocommerce' ), 'labels' => $labels, 'supports' => [ 'title' ], 'hierarchical' => false, 'public' => false, 'show_ui' => true, 'show_in_menu' => false, 'can_export' => true, 'exclude_from_search' => true, 'publicly_queryable' => false, 'capability_type' => 'page' ];
